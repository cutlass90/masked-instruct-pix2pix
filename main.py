...
denoised_latent = pipe(prompt, image=image, num_inference_steps=15, latents=latents, generator=generator, output_type="latent", guidance_scale=10)
with torch.no_grad():
    result_tensor = pipe.vae.decode(denoised_latent.images / pipe.vae.config.scaling_factor, return_dict=False)[0].detach()
